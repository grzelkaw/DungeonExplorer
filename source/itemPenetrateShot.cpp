...
itemPenetrateShot::itemPenetrateShot(const Vector2f& position) : item(position)
{
    if (!texture.loadFromFile("resources/textures/items/penetrateShot.png")) {
        cout << "Brak tekstury penetrateShot";
    }
...
void itemPenetrateShot::ifColliding(player& player, RenderWindow& window)
{
...
void itemPenetrateShot::displayText(player& player, RenderWindow& window)
{
    Text messageText;
